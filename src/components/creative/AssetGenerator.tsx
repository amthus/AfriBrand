...
const AssetGenerator: React.FC<AssetGeneratorProps> = ({ 
  ...
  const exportToPDF = async (asset: Asset) => {
    ...
    // Create a temporary container for a clean export including context
    ...
    exportDiv.innerHTML = `
      <div style="display: flex; justify-between; align-items: center; border-bottom: 2px solid #f1f5f9; padding-bottom: 20px; margin-bottom: 30px;">
        <div>
          <h1 style="font-size: 24px; font-weight: 900; margin: 0;">${dna.name}</h1>
          <p style="font-size: 12px; color: #64748b; margin: 5px 0 0 0;">Brand Identity Asset Export</p>
        </div>
        <div style="text-align: right;">
          <p style="font-size: 10px; font-weight: 800; color: #475569; text-transform: uppercase; letter-spacing: 0.1em; margin: 0;">Campaign</p>
          <p style="font-size: 14px; font-weight: 700; margin: 2px 0 0 0;">${campaign.title}</p>
        </div>
      </div>
      
      <div style="margin-bottom: 30px; border-radius: 20px; overflow: hidden; box-shadow: 0 10px 30px -5px rgba(0,0,0,0.1);">
        <img src="${asset.imageUrl}" style="width: 100%; display: block;" />
      </div>
      
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 40px;">
        <div style="background-color: #f8fafc; padding: 25px; border-radius: 20px;">
          <h3 style="font-size: 10px; font-weight: 800; color: #64748b; text-transform: uppercase; letter-spacing: 0.1em; margin: 0 0 10px 0;">Strategy Notes</h3>
          <p style="font-size: 12px; line-height: 1.6; color: #334155; margin: 0;">${asset.caption}</p>
        </div>
        <div style="background-color: #f8fafc; padding: 25px; border-radius: 20px;">
          <h3 style="font-size: 10px; font-weight: 800; color: #64748b; text-transform: uppercase; letter-spacing: 0.1em; margin: 0 0 10px 0;">Technical specs</h3>
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
            <div>
              <p style="font-size: 9px; color: #94a3b8; margin: 0;">Format</p>
              <p style="font-size: 11px; font-weight: 700; margin: 2px 0 0 0; text-transform: uppercase;">${asset.type}</p>
            </div>
            <div>
              <p style="font-size: 9px; color: #94a3b8; margin: 0;">Ratio</p>
              <p style="font-size: 11px; font-weight: 700; margin: 2px 0 0 0;">${asset.aspectRatio}</p>
            </div>
            <div>
              <p style="font-size: 9px; color: #94a3b8; margin: 0;">Language</p>
              <p style="font-size: 11px; font-weight: 700; margin: 2px 0 0 0;">${asset.language}</p>
            </div>
            <div>
              <p style="font-size: 9px; color: #94a3b8; margin: 0;">Generation</p>
              <p style="font-size: 11px; font-weight: 700; margin: 2px 0 0 0;">${asset.style === 'natural' ? 'AI Optimized' : 'Manual Template'}</p>
            </div>
          </div>
        </div>
      </div>
      
      <div style="margin-top: 40px; text-align: center; border-t: 1px solid #f1f5f9; padding-top: 20px;">
        <p style="font-size: 9px; color: #cbd5e1; letter-spacing: 0.1em; text-transform: uppercase;">Generated by AfriBrand AI • Enterprise Partner</p>
      </div>
    `;
    ...
    try {
      ...
      pdf.save(`${dna.name}_${campaign.title}_Asset.pdf`);
    } finally {
  ...
  const getOverlayForPlatform = (asset: Asset) => {
     ...
     if (platform === 'facebook') {
         return (
             <div className="absolute inset-0 flex flex-col pointer-events-none bg-slate-100">
                 {/* Facebook Header */}
                 <div className="bg-white p-3 border-b border-slate-200 flex items-center gap-2">
                     <div className="w-9 h-9 rounded-full bg-slate-200 flex items-center justify-center text-slate-900 text-xs font-bold ring-1 ring-slate-200">
                        {dna.logoUrl ? <img src={dna.logoUrl} className="w-full h-full object-cover rounded-full" alt="logo" /> : dna.name.charAt(0)}
                     </div>
                     <div className="flex flex-col">
                        <span className="text-[13px] font-bold text-slate-900 leading-tight">{dna.name}</span>
                        <div className="flex items-center gap-1">
                            <span className="text-[11px] text-slate-500 font-medium">Just now • </span>
                            <svg className="w-2.5 h-2.5 text-slate-500" fill="currentColor" viewBox="0 0 16 16"><path d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0zM4.5 7.5a.5.5 0 0 1 .5-.5h6a.5.5 0 0 1 0 1h-6a.5.5 0 0 1-.5-.5z"/></svg>
                        </div>
                     </div>
                     <button className="ml-auto p-1.5 rounded-full hover:bg-slate-100"><svg className="w-4 h-4 text-slate-500" fill="currentColor" viewBox="0 0 16 16"><path d="M3 9.5a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3zm5 0a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3zm5 0a1.5 1.5 0 1 1 0-3 1.5 1.5 0 0 1 0 3z"/></svg></button>
                 </div>
                 ...
                 <div className="bg-white px-3 py-2 flex items-center justify-between border-t border-slate-50">
                     <div className="flex items-center">
                        <div className="w-4 h-4 bg-[#1877f2] rounded-full flex items-center justify-center text-white text-[8px] z-10 border border-white">👍</div>
                        <span className="ml-1.5 text-[11px] text-slate-500 font-medium tracking-tight">42</span>
                     </div>
                     <span className="text-[11px] text-slate-500 font-medium">8 shares</span>
                 </div>

                 {/* Facebook Actions */}
                 <div className="bg-white px-1 pb-1 flex border-t border-slate-100">
                    {[
                        ...
                        { label: 'Share', icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"></path></svg> }
                    ].map(action => (
                        <div key={action.label} className="flex-1 flex items-center justify-center gap-1.5 py-2 hover:bg-slate-50 cursor-pointer pointer-events-auto rounded-md transition-colors text-slate-500">
                            {action.icon}
              ...
              <div key={asset.id} className="group flex flex-col space-y-6">
                ...
                >
                  ...
                  <div className="absolute top-4 right-4 flex flex-col gap-2 items-end z-20">
                    <div className="bg-white/90 text-slate-900 px-3 py-1 rounded-full text-[10px] font-black shadow-md uppercase tracking-widest">{asset.language}</div>
                    {asset.style === 'natural' ? (
                        <div className="bg-brand-600/90 text-white px-3 py-1 rounded-full text-[8px] font-black shadow-md uppercase tracking-[0.2em] flex items-center gap-1.5">
                            <div className="w-1.5 h-1.5 bg-white rounded-full animate-pulse"></div>
                            AI Generated
                        </div>
                    ) : (
                        <div className="bg-slate-900/90 text-white px-3 py-1 rounded-full text-[8px] font-black shadow-md uppercase tracking-[0.2em] flex items-center gap-1.5">
                            Manual Template
                        </div>
                    )}
                  </div>
                  {userRole !== 'Viewer' && asset.type !== 'video' && (
                    <div className="absolute inset-0 bg-slate-900/80 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-8 backdrop-blur-md z-10" data-html2canvas-ignore="true">
                ...
                <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-slate-100 relative group-hover:shadow-amber-100 transition-all">
                  ...
                  <div className="mb-4 space-y-3 bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100">
                    ...
                    <div className="relative">
                      ...
                      {/* Post Actions for Linked accounts */}
                      <div className="flex items-center justify-between border-t border-slate-50 pt-5 gap-3">
                          <div className="flex gap-2">
                            {['instagram', 'facebook', 'whatsapp', 'linkedin'].map(p => {
                              ...
                              const canPost = account?.connected && !isPosted && !isPosting && userRole !== 'Viewer';
                              
                              return (
                                <button 
                                  key={p} 
                                  onClick={() => canPost && handlePostClick(p as SocialPlatform, asset.id)} 
                                  disabled={!canPost} 
                                  className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-[10px] font-black uppercase tracking-wider transition-all shadow-sm ${
                                    isPosted 
                                      ? 'bg-green-100 text-green-700 border-green-200' 
                                      : isPosting
                                        ? 'bg-amber-100 text-amber-700 border-amber-200 animate-pulse'
                                        : account?.connected 
                                          ? 'bg-white text-slate-700 hover:text-amber-600 border-slate-200 hover:border-amber-200 hover:shadow-md' 
                                          : 'bg-slate-50 text-slate-300 border-slate-100 cursor-not-allowed'
                                  } ${userRole === 'Viewer' ? 'opacity-50' : ''}`}
                                  title={!account?.connected ? 'Connect account in settings' : ''}
                                >
                                    {isPosting ? (
                                      ...
                                      <span className="w-1.5 h-1.5 rounded-full bg-slate-300 group-hover:bg-amber-500"></span>
                                    )}
                                    <span>{isPosting ? 'Posting' : isPosted ? 'Posted' : p}</span>
                                </button>
                              );
                            })}
                          </div>
